...
```{r}
# nwkseff=2; quantuse=T; tempo_amu="exp_decay"; thresh=0.4; logtrans=F; ignore.nul=F; decay_init=0; obj="plot"

aucamu = function(nwkseff=2, quantuse=T, tempo_amu="exp_decay", thresh=0.4, logtrans=F, ignore.nul=F, decay_init=0, obj="plot"){
...
run2 = function(gamma, nu, alpha, A, B, k){
  ...
  # return(-sum(dnorm(x=obs, mean=pred_sampl_dates, sd=k, log=T), na.rm=T))
...
```{r}
# fit_mle2 = mle2(minuslogl = run2, start = list(gamma=1, nu=1, alpha=1, A=100, B=1.1, k=10^-5))
