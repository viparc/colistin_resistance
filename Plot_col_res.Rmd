...
plot(x=c(0,0.1), y=c(0,0), xlim=c(0,ncol(col_expo)+5), ylim=c(0,n_cyc+10), type="l", xlab="Weeks", ylab="Cycles", axes=F)
axis(side = 1)
for (i in 1:n_cyc){
 ...
 lines(x = c(0, ncol(col_expo)), y = c(i-0.5, i-0.5), col="grey")
}
rm(i,j)
legend(x=1, y=n_cyc+10, legend=c("Colistin use only", "Any antibiotic excepted colistin", "Colistin and any other antibiotic", "No AMU"), fill=c("red", "blue", "green", "bisque"), cex = 0.8, ncol=3, bty = "n")


# 18th line starting from above
p2 = ggplot(data = gcur[(gcur$SampleID == "0681E"),], aes(x=Time, y=od))
p2 = p2 + xlab("Time (hours)") + ylab("OD")
p2 = p2 + geom_ribbon(aes(x=ifelse((Time>15 & Time<=50), Time, 50), ymin=0, ymax=ifelse((Time>15 & Time<=50), od, od[Time==50])), alpha=1, fill="coral3")
...
run2 = function(aim="est", intr=introd, distrib_res="normal", Sd=0.15, mu, gamma, alpha, teta, delta, beta, eta){ #, lambda){
  ...
  obsdat = aucamu(nwkseff=beta, quantuse=F, tempo_amu="exp_decay", thresh=0.4, logtrans=F, ignore.nul=F, decay_init=0, teta=teta, delta=delta, obj="est")
...
run3 = function(aim="est", intr=introd, distrib_res="normal", mu, gamma, alpha, beta, eta){ #, lambda){
  ...
  obsdat = aucamu(nwkseff=beta, quantuse=F, tempo_amu="exp_decay", thresh=0.4, logtrans=F, ignore.nul=F, decay_init=0, teta=1, delta=1, obj="est")
  obsdat = obsdat[! is.na(obsdat$auc),]
  obsdat$init[is.na(obsdat$init)] = 0
  obsdat$introd = intr
...
fit_mle2 = mle2(minuslogl = run2, method = "Nelder-Mead", control = list(maxit = 5000), start = list(mu = logit(runif(1)), gamma = runif(1), alpha = runif(1), teta = log(runif(1)), delta = logit(runif(1)), beta = logit(runif(1)), eta = logit(runif(1)))) #, lambda = logit(runif(1)))
...
e.step <- function(dat=dat, obs_res, params, type_mod, sigma = 0.15) {
  ...
  if(type_mod == "logistic"){
    ...
    lin_intro = run2(aim="pred", intr=1, mu=params[["mu"]], gamma=params[["gamma"]], alpha=params[["alpha"]], teta=params[["teta"]], delta=params[["delta"]], beta=params[["beta"]], eta=params[["eta"]])[["obsdat"]]$predmod
    lin_nointro = run2(aim="pred", intr=0, mu=params[["mu"]], gamma=params[["gamma"]], alpha=params[["alpha"]], teta=params[["teta"]], delta=params[["delta"]], beta=params[["beta"]], eta=logit(0))[["obsdat"]]$predmod
...
m.step <- function(dat=dat, introd, type_mod) {
  ...
  if(aim == "est"){
    ...
    for(rep in 1:3){
      ...
      fit_mle2[[rep]] = mle2(minuslogl = run2, method = "Nelder-Mead", control = list(maxit = 5000), start = list(mu = logit(runif(1)), gamma = runif(1), alpha = runif(1), teta = log(runif(1)), delta = logit(runif(1)), beta = logit(runif(1)), eta = logit(runif(1))))
      
      ob_pred = run2(aim="pred", mu=coef(fit_mle2[[rep]])[1], gamma=coef(fit_mle2[[rep]])[2], alpha=coef(fit_mle2[[rep]])[3], teta=coef(fit_mle2[[rep]])[4], delta=coef(fit_mle2[[rep]])[5], beta=coef(fit_mle2[[rep]])[6], eta=coef(fit_mle2[[rep]])[7])
...
```{r}
em.2lines <- function(dat=0, tol=10^-3, max.step=1e3, type_mod, init_params) {
  step = 0
  loglik = 10^6
  ...
  repeat {
    ...
    if((counting_for_convergence == 1) | (loglik < -100)){break}
...
obs_vs_pred = run2(aim="pred", intr=results[[3]], mu=coef(results[[1]])[1], gamma=coef(results[[1]])[2], alpha=coef(results[[1]])[3], teta=coef(results[[1]])[4], delta=coef(results[[1]])[5], beta=coef(results[[1]])[6], eta=coef(results[[1]])[7])
