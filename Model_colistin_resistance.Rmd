...
* `viparc_data`: Includes production cycles, AMU, sampling dates in ViParc farms.
...
# viparc_data = read.csv("C:/Users/Jonathan/Downloads/viparc_qualitative.csv")
...
* Case 1 (`case_amu = 1`). We use real counting measures for selected farms, and real cycle lengths. As, for now, only initial samples are available, we simulate the counts for the other samples. In this case, `n_samp` is set to 3 (real value in the study). We focus on colistin use.
...
if (case_amu == 1){
  ...
  for(i in 1:n_cyc){
    ...
    init_flock_size[i] = viparc_data$nb_chicken[which((viparc_data$farm == farm_i)&(viparc_data$flock == flockseq_i)&(viparc_data$week == 1))]
  }
  ...
  rm(i, weeks_use, weeks_no_use, farm_i, flockseq_i, case_amu, weeks_to_keep)
  print("Case 1 chosen.")
...
We plot observations (red dots) and model predictions (black line and CI) for `farms_plot` farms (including 2 farms using colistin): farms 1, 2, 16, 17.

AMU is represented as blue bars.

```{r}
farms_plot = c(1,2,16,17)
```
...
rm(far, farms_plot, iter_list, p, pl)
```


## Model 2: Metapopulation of chicken intestinal microbiotes (not implemented yet)
...
obs_process=function(state, param){
  ...
  n_chick_samp = min(param[[1]], S+R) # Number of individuals whose feces are collected
...
pred_mod_3 = function(farm, param){
  ...
  for (w in 1:n_samp_farm){
    pred_count[w] = obs_process(c(pred_S$y[weeks_samp[farm,w]], pred_R$y[weeks_samp[farm,w]]), list(50, pS, pR))
  }
...
var_f_size = function(flock_size = as.numeric(init_flock_size[3])){
  ...
  for (simu in 1:10000){
    ...
    pred_count = rep(NA, n_samp)
    for (w in 1:n_samp_farm){
      pred_count[w] = obs_process(c(pred_S$y[weeks_samp[3,w]], pred_R$y[weeks_samp[3,w]]), list(50, pS_rand, pR_rand))
    }
