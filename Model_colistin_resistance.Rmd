...
n_samp = 3 # Maximum number of samples during a given cycle
...
* Case 0 (`case_amu` = 0). The true colistin use and sampling dates in farms for which we have AMR data. In this case, `n_samp` is set to 3.
...
We plot the AMU and sampling dates for the `n_cyc` farms.
...
rm(i, farm, n_weeks_farm, n_samp_farm, simul_det, simul_stoch)
```
