...
if (case_amu == 1){
  ...
  for(i in 1:n_cyc){
    ...
    if(length(ab_inc_amu) > 1){
      ...
      weeks_use = viparc_data$week[which((viparc_data$farm == farm_i)&(viparc_data$flock == flockseq_i)&(rowSums(viparc_data[,ab_inc_amu], na.rm=T) !=0))]
      weeks_no_use = viparc_data$week[which((viparc_data$farm == farm_i)&(viparc_data$flock == flockseq_i)&(rowSums(viparc_data[,ab_inc_amu], na.rm=T) ==0))]
      
    }else if(length(ab_inc_amu) == 1){
      
      weeks_use = viparc_data$week[which((viparc_data$farm == farm_i)&(viparc_data$flock == flockseq_i)&(viparc_data[,ab_inc_amu] == T))]
      weeks_no_use = viparc_data$week[which((viparc_data$farm == farm_i)&(viparc_data$flock == flockseq_i)&(viparc_data[,ab_inc_amu] == F))]
