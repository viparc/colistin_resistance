...
$$p(R_i \in M_{intro}) = \frac{exp(\frac{-(r_{intro}^i)^2}{\sigma^2})}{exp(\frac{-(r_{intro}^i)^2}{\sigma^2}) + exp(\frac{-(r_{nointro}^i)^2}{\sigma^2})}$$

Then the introduction status (1/0) of each observation is assigned by drawing in a Bernoulli of parameter $p(R_i \in M_{intro})$.
...
```{r, label="E-M algorithm on one model", fig.width=6, fig.height=4}
...
First, we fit models that do not take into account the introductions, so we do not need to implement the E-M algorithm. Because the package `bbmle` does not always find the likelihood maximum, we repeat the estimation 10 times with `rep_est`, and take the maximum likelihood among these repetitions. The list `lfit` contains all fitted models.
...
We repeat the algorithm 5 times (with different seeds) per model, and each maximization step (model fit) is repeated 5 times (with different seeds too) to make sure the likelihood maximum is reached. This number of repetition is the one generally adopted in MCMC estimation (2-4 chains). These values were also chosen in order to obtain a reasonable computation time on our machine: about 8 hours.
...
The list `lfit` contains the 24 fitted models. We compare and plot their loglikelihood and AIC (adding an interval for showing AIC +/-3). It is possible de zoom on the graph as it is a plotly figure.
...
Model 13 has the lowest AIC. Models 14, 15 and 19 seem to be equivalent in term of AIC (the -/+3 interval includes the value of model 13). Models 16, 20 and 21 also have close AIC values.

We perform Likelihood Ratio Tests in these models, pair by pair, when they are nested.

```{r}
bbmle::anova(lfit[[13]], lfit[[14]])
bbmle::anova(lfit[[13]], lfit[[15]])
bbmle::anova(lfit[[13]], lfit[[16]])
bbmle::anova(lfit[[13]], lfit[[20]])
bbmle::anova(lfit[[13]], lfit[[21]])

```

Model 13 seems to explain best the data, and it is not significant to add variables to complexify model 13.

Model 13 is the model that takes only account the introductions (no AMU, no effect of the previous resistance). It is likely that the E-M algorithm simply assigns a status of introduction when the observed resistance is high, and a status of no-introduction when the observed resistance is low. It seems uncertain wether it is because AMU and previous resistance have no effect.

#### Analysis alternative

A way to perform the analysis would be to:

1) Compare models without introduction (models 1 to 12)
2) Among the best of them (lowest AIC), compute the E-M algorithm to check if adding introductions improves the fit
3) Compare the models that are left

Here, among models 1-12, we would select models 7, 8 and 9, and perform the E-M algorithm with them only. In the end, we would therefore compare models 7, 8, 9, 19, 20 and 21.

Models 19, 20 and 21 have lower - and quite similar - AIC. Lets us perform Likelihood Ratio Tests:

```{r}
bbmle::anova(lfit[[19]], lfit[[20]])
bbmle::anova(lfit[[19]], lfit[[21]])

```

In this case, model 19 is the final model chosen. It includes an effect of the introductions and recent AMU, but no effect of the previous resistance and of the initial AMU.
