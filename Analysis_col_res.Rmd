...
resamu = function(nwkseff, quantuse=amu_model_is_quanti, tempo_amu="exp_decay", ignore.nul=F, lag_amu_init, obj="plot", expo, var_to_plot = "amu_t"){
  ...
  for(cyc_i in 1:n_cyc){
    for(time_i in 2:3){
      ...
      first_amu_week_consid = max(1, weeks_samp[cyc_i, time_i] - nwkseff - 1)
      ...
      if(tempo_amu %in% c("step", "linear")){
        inc_wks = floor(first_amu_week_consid):last_amu_week_consid
      ...
      if((tempo_amu %in% c("step", "linear")) & (ceil(first_amu_week_consid) - first_amu_week_consid != 0)){
        amu_t[floor(first_amu_week_consid)] = amu_t[floor(first_amu_week_consid)] * (ceil(first_amu_week_consid) - first_amu_week_consid)
      }
...
run2 = function(aim="est", intr=introd, mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd){
  ...
  # obsdat$glm_predmod_intro = pmax(pmin(obsdat$glm_predmod_intro, 0.999), 0.001)
  ...
  # obsdat$glm_predmod_nointro = pmax(pmin(obsdat$glm_predmod_nointro, 0.999), 0.001)
...
fitplot = function(fitted_model, intro, var_to_plot = "amu_t"){
  ...
  if(type_mod == "logis"){
    ...
    set.seed(78)
    p2 = p2 + geom_jitter(data = obs_vs_pred[[1]], aes(x = amu_t, y = resquanti, col = introduc, fill = predmod, shape = (predmod>=thresh_res)), size = 3, height = 0.1, width = 0)
...
if(test_method_comp){
  ...
  res_data = data.frame(S = res_data_saved$S, M = rnorm(n_cyc, 0, 0.05) + base::sample(x = c(0.2,0.7), size = n_cyc, replace = T, prob = c(0.5,0.5)), E = rnorm(n_cyc, 0, 0.05) + base::sample(x = c(0.2,0.7), size = n_cyc, replace = T, prob = c(0.5,0.5)))
...
if(test_method_comp){
  ...
  test_scenario_1 = list(res_data, results_mod13, results_mod19)
...
if(test_method_comp){
  ...
  high_use_low_res = 35 #sample(x = which(fake_amu >= 0.5), size = 1)#42 #NULL
  ...
  if(length(na_res_val) != 1){
    print("CAUTION: The number of NAs in the true resistance data on 2nd and 3rd samplings is different from 1.")
  }else{
...
if(test_method_comp){
  results_mod13 = test_scenario_2[[2]]
  results_mod19 = test_scenario_2[[3]]
}else{
  ...
  test_scenario_2 = list(res_data, results_mod13, results_mod19)
...
if(test_method_comp){
  ...
  low_use_high_res = NULL #sample(x = which(fake_amu < 0.5), size = round(0.3*sum(fake_res < 0.5)))
  fake_res[low_use_high_res] = rnorm(length(low_use_high_res), 0.8, 0.05)
...
if(test_method_comp){
  results_mod13 = test_scenario_3[[2]]
  results_mod19 = test_scenario_3[[3]]
}else{
  results_mod13 = EM_algo(modnum=13, numb_rep_algo=1)

  results_mod19 = EM_algo(modnum=19, numb_rep_algo=1)
  
  test_scenario_3 = list(res_data, results_mod13, results_mod19)
...
if(alread_comp){
  ...
  clusterExport(cl, c("effect_recent_amu","val_eta","thresh_res","type_mod","definition_amu","amu_model_is_quanti","E_step","M_step","rep_est","run2","run2_intr","resamu","n_cyc","res_data","weeks_samp","col_expo","col_expo_quanti","allab_expo","allab_expo_quanti"))
  resulpar = parLapply(cl, X=1:12, fun=EM_algo, numb_rep_algo=1, plot_step=F)
  stopCluster(cl)
...
```{r}
print(tab_comp_mod$AIC_weights[2] /tab_comp_mod$AIC_weights[1])
print(tab_comp_mod$AIC_weights[4] /tab_comp_mod$AIC_weights[1])
print(tab_comp_mod$AIC_weights[7] /tab_comp_mod$AIC_weights[1])
print(tab_comp_mod$AIC_weights[13] /tab_comp_mod$AIC_weights[1])

```

```{r}
print(tab_comp_mod$AIC_weights[best_model] /tab_comp_mod$AIC_weights[13])
bbmle::anova(resulpar[[best_model]][[1]], resulpar[[1]][[1]])
