...
$$A_{recent,i}(t)=\sum_{j=1}^{t-1}(U(j).exp[-\frac{ln(1/\epsilon)}{\beta}.(t-j-1)])$$
...
As shown in the figure below, $A_{recent,i}(t)$ is parametrized as an exponential decay: after $\beta$ weeks, the effect is as low as $\epsilon$, that we fix at 5%.
...
if(alread_comp){
  load(paste0("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/", definition_amu, " - Qualitative - Exp/lfit.rdata"))
}else{
  introd = 0
  lfit = list()
...
if(alread_comp){
  load(paste0("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/", definition_amu, " - Qualitative - Exp/resulpar.rdata"))
}else{
  ...
  resulpar_13to21 = parLapply(cl, X=13:21, fun=EM_algo, numb_rep_algo=5)
  stopCluster(cl)
  save(resulpar_13to21, file = "C:/Users/Jonathan/Desktop/resulpar_13to21.rdata")
  
  cl<-makeCluster(3)
  clusterEvalQ(cl, list(library(pracma), library(bbmle)))
  clusterExport(cl, c("definition_amu","E_step","M_step","rep_est","run2","run2_intr","resamu","n_cyc","res_data","weeks_samp","col_expo","col_expo_quanti","allab_expo","allab_expo_quanti"))
  resulpar_22to24 = parLapply(cl, X=22:24, fun=EM_algo, numb_rep_algo=5)
  stopCluster(cl)
  save(resulpar_22to24, file = "C:/Users/Jonathan/Desktop/resulpar_22to24.rdata")
...
plot_aic_ll = data.frame(mod = as.factor(1:24), LogLikelihood = l_loglik, MinAIC = -l_aic, AIC_weights = l_aic_weights)
...
### Correlation between the probability of introduction and the geographical location
...
```{r}
prob_intro_farms = data.frame(Farm = resulpar[[1]][[7]], Prob = resulpar[[1]][[3]])
...
plot(x = val_exp, y = sapply(X = val_exp, FUN = coef_moran, decay_fun = "exp")["max_dist",], type="l", main = "Maximum distance such as W(i,j) > 0.05", xlab = "k", ylab = "Distance (°)")
