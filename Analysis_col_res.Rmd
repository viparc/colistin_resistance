...
run2 = function(aim="est", intr=introd, mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd){
  ...
  if(type_mod == "linear"){
    ...
    # total_loglik = dnorm(x=obsdat$obsres, mean=obsdat$predmod, sd=Sd, log=T)
    ...
    if(aim == "est"){
      ...
      return(list(obsdat = obsdat[,c("obsres", "amu_init", "amu_t", "introduc", "predmod", "cycle", "predmod_inf", "predmod_sup")], minll = -sum(total_loglik), true_param = c(mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd)))
    }
  
  }else if(type_mod == "log_beta"){
    
    obsdat$glm_predmod_intro = mu + lambda1 * obsdat$init + (lambda2 * obsdat$prev)*as.numeric(obsdat$end) + alpha * obsdat$amu_t + teta * obsdat$amu_init + eta
    obsdat$glm_predmod_intro = pracma::sigmoid(obsdat$glm_predmod_intro)
    ...
    # total_loglik = (obsdat$predmod^obsdat$obsres)*((1-obsdat$predmod)^(1-obsdat$obsres))
    total_loglik = dbeta(x=obsdat$obsres, shape1=Sd, shape2=Sd*(1.00001-obsdat$predmod)/(obsdat$predmod+0.00001), log=T)
    total_loglik[is.infinite(total_loglik)] = -710
    ...
    if(aim == "est"){
      ...
      obsdat$predmod_inf = qbeta(p = 0.025, shape1 = Sd, shape2 = Sd*(1.00001-obsdat$predmod)/(obsdat$predmod+0.00001))
      obsdat$predmod_sup = qbeta(p = 0.975, shape1 = Sd, shape2 = Sd*(1.00001-obsdat$predmod)/(obsdat$predmod+0.00001))
...
fitplot = function(fitted_model, intro){
  ...
  # p2 = ggplot(obs_vs_pred[[1]], aes(x = obsres, y = predmod - obsres, fill = true_fit_coef["alpha"]*amu_t + true_fit_coef["teta"]*amu_init, col = introduc))
  # p2 = p2 + geom_hline(yintercept = 0, linetype = "dashed")
  # p2 = p2 + geom_point(shape = 21, size = 3)
  # plot(p2)
  
  # plot(qqnorm(obs_vs_pred[[1]]$predmod - obs_vs_pred[[1]]$obsres))
}
```
...
rep_est = function(nrep, minusloglrep, methodrep, controlrep, fixedvar, startvar){
  ...
  for(repet in 1:nrep){
    ...
    if(type_mod == "linear"){
      ...
      valpar[startvar] = runif(length(startvar), min = -10, max = 10)
    }
...
```{r}
E_step_introperfarm <- function(observed, params) {

  sigma = 3 * sigmoid(params[["Sd"]]) # because the parameter is modified when estimated in function "run2"
  
  lin_intro = run2(aim="pred", intr=1, mu=params[["mu"]], lambda1=params[["lambda1"]], lambda2=params[["lambda2"]], alpha=params[["alpha"]], beta=params[["beta"]], teta=params[["teta"]], delta=params[["delta"]], eta=params[["eta"]], Sd=params[["Sd"]])[["obsdat"]][, c("predmod","cycle")]
  
  lin_nointro = run2(aim="pred", intr=0, mu=params[["mu"]], lambda1=params[["lambda1"]], lambda2=params[["lambda2"]], alpha=params[["alpha"]], beta=params[["beta"]], teta=params[["teta"]], delta=params[["delta"]], eta=params[["eta"]], Sd=params[["Sd"]])[["obsdat"]][, c("predmod","cycle")]

  # Residuals of observations under models "Intro" and "No intro"
  lin_intro$r = (abs(lin_intro$predmod - observed))^2
  lin_nointro$r = (abs(lin_nointro$predmod - observed))^2

   # Adds a column with the sum of r_intro for the 2 (or 1) sample of the same cycle:
  lin_intro = merge(x = lin_intro, by.x = "cycle", y = stats::aggregate(x = lin_intro$r, by = list(lin_intro$cycle), FUN = sum), by.y = "Group.1")
  lin_nointro = merge(x = lin_nointro, by.x = "cycle", y = stats::aggregate(x = lin_nointro$r, by = list(lin_nointro$cycle), FUN = sum), by.y = "Group.1")
  
  # Residuals follow a normal distribution
  exp_intro = exp(- lin_intro$x/(2*sigma^2))
  exp_nointro = exp(- lin_nointro$x/(2*sigma^2))
...
EM_algo <- function(modnum, numb_rep_algo=3, tol=1, max.step=30) {
  ...
  for(algorep_ite in 1:numb_rep_algo){
    ...
    while((counting_for_convergence <2) & (step <= max.step+1)){
      ...
      # If we dtermine a prior value for "eta", because it could not be estimated in the first step (no introduction)
      # if(step == 1){
      #   raw_params["eta"] = log(0.1)
      # }
...
plot(hist(as.numeric(as.matrix(res_data)), nclass = 15), col = "royalblue4", main = "Histogram of all resistance data", xlab = "Resistance values")
