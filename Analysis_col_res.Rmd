...
resamu = function(nwkseff, quantuse=T, tempo_amu="exp_decay", thresh_res, ignore.nul=F, lag_amu_init, obj="plot", expo="allab"){
  ...
  if(obj == "plot"){
    p = ggplot(data=obtab, aes(x=amu_t, y=auc, col=init))
...
If the computation was already done, `alread_comp = T`.
...
We repeat the algorithm 5 times (with different seeds) per model, and each maximization step (model fit) is repeated 5 times (with different seeds too) to make sure the likelihood maximum is reached. These values were also chosen in order to obtain a reasonable computation time on our machine.
...
In conclusion, model 19 (introductions + recent AMU) seem to explain best the data, followed closely by model 13 (introductions only). With the LRT, it is not significant to add variables to complexify model 13.

### Analysis alternative

A way to perform the analysis could also be to:
