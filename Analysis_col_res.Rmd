...
if(type_res_data == "growth_curves"){
  ...
  p = p + xlab("Resistance values") + ylab("Count")
  plot(p)
...
p2 = p2 + geom_ribbon(aes(x=Time, ymin=0, ymax=od), alpha=1, fill="chartreuse4")
...
resamu = function(nwkseff, quantuse=amu_model_is_quanti, tempo_amu="exp_decay", ignore.nul=F, lag_amu_init, obj="plot", expo){
  ...
  if(obj == "plot"){
    p = ggplot(data=obtab, aes(x=amu_t, y=resquanti, col=init))
...
run2 = function(aim="est", intr=introd, mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd){
  ...
  if(type_mod == "linear"){
    ...
    if(sum(intr) == 0){
      ...
      weights_lkld =  1 - ((cor(obsdat$predmod, obsdat$introduc))^2)/4 #1-obsdat$introduc
    }
    total_loglik = log(weights_lkld*(obsdat$predmod^obsdat$resquali)*((1-obsdat$predmod)^(1-obsdat$resquali))) #log(weights_lkld*(obsdat$predmod^obsdat$resquali)*((1-obsdat$predmod)^(1-obsdat$resquali)) /sum(weights_lkld))
    total_loglik[is.infinite(total_loglik)] = -710
...
E_step <- function(observed, params) {
  ...
  pred_intro = 1 #run2(aim="pred", intr=1, mu=params[["mu"]], lambda1=params[["lambda1"]], lambda2=params[["lambda2"]], alpha=params[["alpha"]], beta=params[["beta"]], teta=params[["teta"]], delta=params[["delta"]], eta=params[["eta"]], Sd=params[["Sd"]])[["obsdat"]]$predmod
  ...
  if(type_mod == "linear"){
    ...
    prob_intro = 1 - (r_intro/ (r_intro + r_nointro))
...
M_step <- function(introd, modnum, prevfit) {
  ...
  if(modnum == 13){
    ...
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c( "lambda1", "alpha",  "beta",  "teta",  "delta", "Sd", "eta"), startvar = c("mu",  "lambda2"))
  }else if(modnum == 16){
    ...
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c( "lambda1",  "alpha",  "beta", "Sd", "eta"), startvar = c("mu",  "lambda2",  "teta",  "delta"))
  }else if(modnum == 19){
    ...
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c( "lambda1",  "teta",  "delta", "Sd", "eta"), startvar = c("mu",  "lambda2",  "alpha",  "beta"))
  }else if(modnum == 22){
    ...
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c( "lambda1",  "Sd", "eta"), startvar = c("mu",  "lambda2",  "alpha",  "beta",  "teta",  "delta"))
  }
  #####
  ...
  print(true_coef)
  print(1-cor(ob_pred[[1]]$predmod, ob_pred[[1]]$introduc)^2)
...
```{r}
EM_algo <- function(modnum, numb_rep_algo=5, tol=0.1, max.step=50) {
  ...
  for(algorep_ite in 1:numb_rep_algo){
    ...
    while((step < 4) | ((counting_for_convergence <2) & (step <= max.step+1))){
      step = step +1
      if(step == max.step+1){
      ...
      introductions_next_step = E_step(observed=obs_res, params=raw_params)
...
if(test_method_comp){
  ...
  if(type_mod == "linear"){
    ...
    high_use_low_res = NULL #sample(x = which(fake_amu >= 0.5), size = 1)#35
...
if(test_method_comp){
  ...
  if(type_mod == "linear"){
    ...
    high_use_low_res = NULL #42 #sample(x = which(fake_amu >= 0.5), size = 1) #35
    fake_res[high_use_low_res] = rnorm(length(high_use_low_res), 0.2, 0.05)
...
if(alread_comp){
  ...
  lfit[[3]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c( "lambda1",  "alpha",  "beta",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "lambda2"))
  ...
  lfit[[6]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c( "lambda1",  "alpha",  "beta", "eta", "Sd"), startvar = c("mu",  "lambda2",  "teta",  "delta"))
  ...
  lfit[[9]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c( "lambda1",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "lambda2",  "alpha",  "beta"))
  ...
  lfit[[12]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "eta", "Sd"), startvar = c("mu", "lambda2",  "alpha",  "beta",  "teta",  "delta"))
...
local_moran = function(decay_param, decay_fun, p_val, style_weight){
  ...
  p1 = p1 + geom_point(data = prob_intro_farms, aes(x = Long, y = Lat, fill = cluster), col = "black", shape = 21, size = 4)
  ...
  p2 = p2 + geom_smooth(method = 'lm', formula = y~x, col = "red", linetype = "dashed")
