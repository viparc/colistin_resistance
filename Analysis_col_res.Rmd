...
if(type_res_data == "growth_curves"){
  ...
  for (concentr in c(0.5, 1, 2, 4, 8, 12, 16)){
    ...
    # mean_this_time_conc_0 = rep(aggregate(gcur[gcur$Conc. == 0, c("Time", "value")], by = list(gcur$Time[gcur$Conc. == 0]), FUN = mean)$value, each = sum((gcur$Time == 1)&(gcur$Conc. == concentr)))
    # gcur$value[gcur$Conc. == concentr] = gcur$value[gcur$Conc. == concentr] - mean_this_time_conc_0
...
For these concentrations (1, 2 and 4), we compute the area under curve (AUC), for each sample, between t=1 hours and t=79 hours.
...
if(type_res_data == "growth_curves"){
  ...
  VdE_A_score = function(conc, t_start, t_end){
    ...
    list(agreement(table(factor(round(all_auc), levels = (min(round(all_auc)): max(round(all_auc)))))), all_auc, sd(all_auc))
  }
  ...
  p = p + geom_histogram(bins = 15, fill = "royalblue4", alpha = 0.5)
  p = p + geom_jitter(aes(y = 3), col = "black", size = 1, width = 0, height = 3)
  p = p + geom_vline(data = distrib_auc[distrib_auc$conc == 1,], aes(xintercept = -33), size = 1, col = "red", linetype = "dashed")
  p = p + facet_wrap(~conc)
  p = p + ggtitle("Distribution of resistance metric values for each concentration (in mg/L)")
  p = p + xlab("Resistance metric (area between growth curve and reference)") + ylab("Count")
  p = p + theme_bw()
  plot(p)
  
  # mixtmod = normalmixEM(distrib_auc$val[distrib_auc$conc == 1], k=2)
  # group1 = mixtmod$lambda[1] * rnorm(5000, mixtmod$mu[1], mixtmod$sigma[1])
  # group2 = mixtmod$lambda[2] * rnorm(5000, AA$mu[2], AA$sigma[2])
  # p = ggplot(data = distrib_auc[distrib_auc$conc == 1,], aes(x = val))
  # p = p + geom_density()
  # p = p + geom_density(aes(x = group1), col = "red", size = 2)
  # p = p + geom_density(aes(x = group2), col = "blue", size = 2)
  # p
  ...
  for(flo in unique(gcur$FlockID)){
    for(sp in unique(gcur$SamplPoint)){
      if(length(1:79) == length(gcur$od[which((gcur$Time %in% (1:79)) & (gcur$FlockID == flo) & (gcur$SamplPoint == sp))])){
        ...
        auc_ob[flo, sp] = trapz(x = (1:79), y = gcur$od[which((gcur$Time %in% (1:79)) & (gcur$FlockID == flo) & (gcur$SamplPoint == sp))])
      }else{
...
resamu = function(nwkseff, quantuse=amu_model_is_quanti, tempo_amu="exp_decay", ignore.nul=F, lag_amu_init, obj="plot", expo, var_to_plot = "amu_t"){
  ...
  for(cyc_i in 1:n_cyc){
    for(time_i in 2:3){
      ...
      if(quantuse){
        ...
        amu_init = c(use_quali[cyc_i, (1:floor(lag_amu_init))], (lag_amu_init - floor(lag_amu_init)) * use_quali[cyc_i, ceiling(lag_amu_init)])
      }
  ...
  obtab$resquali = (obtab$resquanti >= thresh_res)
...
server <- function(input, output){
  ...
  output$plotdisp <- renderPlot({
    ...
    # plot
    resamu(nwkseff=input$nwkseff, lag_amu_init=input$lag_amu_init, expo=input$expo, quantuse=input$quantuse, tempo_amu=input$tempo_amu, ignore.nul=input$ignore.nul)
  })
}
...
# p = p + ggtitle("Metric for the past use of antibiotics (Si and Lij)")
p = p + xlab("Weeks k") + ylab("Effect of antimicrobial use")
...
p = p + geom_text(aes(x=6, y=0.15, label = "tij-beta"), parse = T, size=5, col="blue")
p = p + geom_text(aes(x=10, y=1, label = "tij-1"), parse = T, size=5, col="black")
...
p = p + scale_color_manual(name="", values=c("1"="blue", "2"="red"), labels=c("1"="Lij (recent AMU)", "2"="Si (initial AMU)"))
p = p + theme_bw()
p
...
```{r}
run2 = function(aim="est", intr=introd, mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd){

  beta = 10 * sigmoid(beta) +1
  delta = 4 * sigmoid(delta) +1
  ...
  if(type_mod == "linear"){
    ...
    eta = sigmoid(eta)
    Sd = 3 * sigmoid(Sd)
  }else if(type_mod == "logis"){
    eta = 0.99999999
    Sd = 1
  }

  obsdat = resamu(nwkseff=beta, quantuse=amu_model_is_quanti, tempo_amu="exp_decay", ignore.nul=F, lag_amu_init=delta, obj="est", expo=definition_amu)
  ...
  if(type_mod == "linear"){
    ...
    obsdat$predmod_intro = mu + eta # mu + lambda1 * obsdat$init + (lambda2 * obsdat$prev)*as.numeric(obsdat$end) + alpha * obsdat$amu_t + teta * obsdat$amu_init + eta
    ...
    # total_loglik = dnorm(x=obsdat$resquanti, mean=obsdat$predmod, sd=Sd, log=T)
    lik_intro = dnorm(x=obsdat$resquanti, mean=obsdat$predmod_intro, sd=Sd)
    lik_nointro = dnorm(x=obsdat$resquanti, mean=obsdat$predmod_nointro, sd=Sd)
    ...
    if(aim == "est"){
      ...
      obsdat$predmod_inf = qnorm(p = 0.025, mean=obsdat$predmod, sd=Sd)
      obsdat$predmod_sup = qnorm(p = 0.975, mean=obsdat$predmod, sd=Sd)
      
      return(list(obsdat = obsdat[,c("resquanti", "amu_init", "amu_t", "introduc", "predmod", "cycle", "predmod_inf", "predmod_sup", "init")], minll = -sum(total_loglik), true_param = c(mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd)))
    }
  
  }else if(type_mod == "logis"){
    
    obsdat$init = (obsdat$init >= thresh_res)
    obsdat$prev = (obsdat$prev >= thresh_res)
    # obsdat$amu_init = (obsdat$amu_init >0)
    ...
    obsdat$predmod = obsdat$introduc * obsdat$glm_predmod_intro + (1-obsdat$introduc) * obsdat$glm_predmod_nointro #(1-obsdat$introduc) * obsdat$glm_predmod_nointro
    
    if(sum(intr) == 0){
      weights_lkld = 1-obsdat$introduc
    }else{
      # weights_lkld =  1 - ((cor(obsdat$predmod, obsdat$introduc))^2)/4
      weights_lkld =  1-obsdat$introduc
    }
    # total_loglik = log(weights_lkld*(obsdat$predmod ^ obsdat$resquali)*((1-obsdat$predmod) ^ (1-obsdat$resquali))/sum(weights_lkld))
...
total_loglik[is.infinite(total_loglik)] = -710
    ...
    if(aim == "est"){
      ...
      return(list(obsdat = obsdat[,c("resquali", "amu_init", "amu_t", "introduc", "predmod", "cycle", "resquanti", "init", "prev")], minll = -sum(total_loglik), true_param = c(mu, lambda1, lambda2, alpha, beta, teta, delta, eta=eta, Sd=Sd)))
    }
  }
}
...
run2_intr <- function(x) {
 function(aim="est", mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd)
   run2(aim, intr=x, mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd)
}
...
fitplot = function(fitted_model, intro){
  ...
  if(type_mod == "linear"){
    p = ggplot(obs_vs_pred[[1]], aes(x = resquali, y = predmod, fill = true_fit_coef["alpha"]*amu_t + true_fit_coef["teta"]*amu_init, col = introduc))
  }else if(type_mod == "logis"){
    p = ggplot(obs_vs_pred[[1]], aes(x = resquanti, y = predmod, fill = true_fit_coef["alpha"]*amu_t + true_fit_coef["teta"]*amu_init, col = introduc))
    p = p + geom_vline(xintercept = thresh_res, linetype = "dashed")
    p = p + geom_hline(yintercept = thresh_res, linetype = "dashed")
  }
  ...
  if(type_mod == "linear"){
    p = p + geom_errorbar(aes(ymin = predmod_inf, ymax = predmod_sup), col = "grey")
  }
  ...
  if(type_mod == "logis"){
    ...
    p2 = p2 + geom_point(data = obs_vs_pred[[1]], aes(x = amu_t, y = resquanti, col = introduc, fill = predmod, shape = (predmod>=thresh_res)), size = 3)
    ...
    p3 = ggplot()
    p3 = p3 + xlab("Recent AMU") + ylab("Observed resistance (quali)")
    p3 = p3 + ggtitle("Resistance = f(recent amu)")
    p3 = p3 + geom_line(data=for_curve, aes(x = amu, y = true_fit_coef["eta"]), col = "red")
    p3 = p3 + geom_line(data=for_curve, aes(x = amu, y = amr), col = "green")
    p3 = p3 + geom_point(data = obs_vs_pred[[1]], aes(x = amu_t, y = as.numeric(resquali), col = introduc, shape = (predmod>=thresh_res)), size = 3)
    p3 = p3 + scale_color_gradient(name = "Probability of introduction", low = "green", high = "red")
    p3 = p3 + scale_shape_manual(name = "Predicted resistance (quali)", values = c(16, 17))
    plot(p3)
    
    print(paste0("Sensitivity: ", sum((obs_vs_pred[[1]]$predmod >= thresh_res) & (obs_vs_pred[[1]]$resquali))/sum(obs_vs_pred[[1]]$resquali)))
    print(paste0("Specificity: ", sum((obs_vs_pred[[1]]$predmod >= thresh_res) & (obs_vs_pred[[1]]$resquali))/sum(obs_vs_pred[[1]]$predmod >= thresh_res)))
  }
}
  ...
  for(repet in 1:nrep){
    ...
    if(prev_est[1] == "none"){
      ...
      if(type_mod == "linear"){
        ...
        valpar[startvar] = runif(length(startvar), min = -100, max = 100)
...
$$Q(\theta,\theta^t) = E[ln \, P(Y,Z/\theta) \quad /Y,\theta^t]=\sum_{m} E[ln \, P(Y_m,Z_m/\theta) \quad /Y_m,\theta^t]$$

$$Q(\theta,\theta^t) = \sum_{m} [P(Z_m=0/Y_m,\theta^t).ln \, P(Z_m=0,Y_m/\theta) \: + \: P(Z_m=1/Y_m,\theta^t).ln \, P(Z_m=1,Y_m/\theta)]$$

Therefore:
$$\begin{align*}
P(Z_m=1/Y_m,\theta^t) &= \frac{P(Z_m=1,Y_m/\theta^t)}{P(Y_m/\theta^t)}\\
                      &= \frac{P(Z_m=1,Y_m/\theta^t)}{P(Z_m=0,Y_m/\theta^t) \: + \: P(Z_m=1,Y_m/\theta^t)}\\
                      &= \frac{\eta^{R_m}.(1-\eta)^{1-R_m}}{M_{m,t}^{R_m}.(1-M_{m,t})^{1-R_m} \: + \: \eta^{R_m}.(1-\eta)^{1-R_m}}
\end{align*}$$ 
                      
$$P(Z_m=0/Y_m,\theta^t) = 1-P(Z_m=1/Y_m,\theta^t)$$

And:

$$\begin{align*}
  Q(\theta,\theta^t) &= \sum_{m} [P(Z_m=0/Y_m,\theta^t).ln \, P(Z_m=0,Y_m/\theta) \: + \: P(Z_m=1/Y_m,\theta^t).ln \, P(Z_m=1,Y_m/\theta)]\\
                     &= \sum_{m} [P(Z_m=0/Y_m,\theta^t).ln \, [P(Z_m=0/Y_m,\theta).P(Y_m/\theta)] \: + \: P(Z_m=1/Y_m,\theta^t).ln \, [P(Z_m=1/Y_m,\theta).P(Y_m/\theta)]]\\
                     &= \sum_{m} [[ln \, P(Y_m/\theta)].[P(Z_m=0/Y_m,\theta^t)+P(Z_m=1/Y_m,\theta^t)]\\
                     &\: + \: P(Z_m=0/Y_m,\theta^t).ln \, P(Z_m=0/Y_m,\theta) \: + \: P(Z_m=1/Y_m,\theta^t).ln \, P(Z_m=1/Y_m,\theta)]\\
                     &= \sum_{m} [ln \, P(Y_m/\theta)] \: + \: H(\theta,\theta^t)\\
                     &= L_{obs}(\theta) \: + \: H(\theta,\theta^t)\\
\end{align*}$$

$$H(\theta,\theta^t)= \sum_{m} [P(Z_m=0/Y_m,\theta^t).ln \, P(Z_m=0/Y_m,\theta) \: + \: P(Z_m=1/Y_m,\theta^t).ln \, P(Z_m=1/Y_m,\theta)]$$

$$Q(\theta^*,\theta^*)= L_{obs}(\theta^*) \: + \: H(\theta^*,\theta^*)$$

$$IC_{H,Q}=-2Q(\theta^*,\theta^*)+2H(\theta^*,\theta^*)+C^*(\theta^*)$$

$$H(\theta^*,\theta^*)= \sum_{m} [P^{cont}_{m}.ln(P^{cont}_{m})]$$
...
E_step <- function(observed, params, prev_p_intro) {
  ...
  pred_intro = 0.99 #params[["eta"]] #run2(aim="pred", intr=1, mu=params[["mu"]], lambda1=params[["lambda1"]], lambda2=params[["lambda2"]], alpha=params[["alpha"]], beta=params[["beta"]], teta=params[["teta"]], delta=params[["delta"]], eta=params[["eta"]], Sd=params[["Sd"]])[["obsdat"]]$predmod

  pred_nointro = run2(aim="pred", intr=0, mu=params[["mu"]], lambda1=params[["lambda1"]], lambda2=params[["lambda2"]], alpha=params[["alpha"]], beta=params[["beta"]], teta=params[["teta"]], delta=params[["delta"]], eta=params[["eta"]], Sd=params[["Sd"]])[["obsdat"]]$predmod
  
  # Residuals of observations under models "Intro" and "No intro"
  r_intro = abs(pred_intro - observed)
  r_nointro = abs(pred_nointro - observed)
  ...
  if(type_mod == "linear"){
    ...
    observed = as.numeric(observed >= thresh_res)
    pred_nointro = pmax(pmin(pred_nointro, 0.99), 0.01)
    ...
    # prob_intro = observed*(observed - pred_nointro)#/(observed + pred_nointro) # 1 - (r_intro/ (r_intro + r_nointro))
    # prob_intro = pmax(pmin(prob_intro, 0.99), 0.01)
    # prob_intro = sigmoid(20* (0.5 - (r_intro/ (r_intro + r_nointro))))
    # prob_intro[which(observed < pred_nointro)] = 0
    # prob_intro[which((prob_intro >0.4) & (prob_intro <0.6))] = 0.5
    # prob_intro[prob_intro >0.5] = 1
    # prob_intro[prob_intro <0.5] = 0
  }
  # prob_intro = exp_intro / (exp_intro + exp_nointro)
...
M_step <- function(introd, modnum, prevfit) {
  ...
  #####
  if(modnum == 13){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2",  "alpha",  "beta",  "teta",  "delta", "Sd", "eta"), startvar = c("mu"))
  }else if(modnum == 14){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2",  "alpha",  "beta",  "teta",  "delta", "Sd", "eta"), startvar = c("mu",  "lambda1"))
  }else if(modnum == 15){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("alpha",  "beta",  "teta",  "delta", "Sd", "eta"), startvar = c("mu", "lambda1",  "lambda2"))
  }else if(modnum == 16){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2",  "alpha",  "beta", "Sd", "eta"), startvar = c("mu",  "teta",  "delta"))
  }else if(modnum == 17){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2",  "alpha",  "beta", "Sd", "eta"), startvar = c("mu",  "lambda1",  "teta",  "delta"))
  }else if(modnum == 18){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("alpha",  "beta", "Sd", "eta"), startvar = c("mu",  "lambda1",  "lambda2",  "teta",  "delta"))
  }else if(modnum == 19){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2",  "teta",  "delta", "Sd", "eta"), startvar = c("mu",  "alpha",  "beta"))
  }else if(modnum == 20){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2",  "teta",  "delta", "Sd", "eta"), startvar = c("mu",  "lambda1",  "alpha",  "beta"))
  }else if(modnum == 21){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("teta",  "delta", "Sd", "eta"), startvar = c("mu",  "lambda1",  "lambda2",  "alpha",  "beta"))
  }else if(modnum == 22){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2", "Sd", "eta"), startvar = c("mu",  "alpha",  "beta",  "teta",  "delta"))
  }else if(modnum == 23){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2", "Sd", "eta"), startvar = c("mu",  "lambda1",  "alpha",  "beta",  "teta",  "delta"))
  }else if(modnum == 24){
    modstep = rep_est(nrep=3, minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("Sd", "eta"), startvar = c("mu",  "lambda1",  "lambda2",  "alpha",  "beta",  "teta",  "delta"))
  }
  #####
...
```{r}
EM_algo <- function(modnum, numb_rep_algo=5, tol=0.001, max.step=100000000000) {
  ...
  for(algorep_ite in 1:numb_rep_algo){
    ...
    introductions_next_step = 0.01
    raw_params = "none"
    
    obs_res = resamu(obj="est", nwkseff=1, lag_amu_init=1, expo=definition_amu)$resquanti
    ...
    while((step < 4) | ((counting_for_convergence <5) & (step <= max.step+1))){
      ...
      # fitplot_for_EMalgo(modstep, introductions_next_step)
    ...
    # print("Evaluating DM matrix...")
    # penal_term = rep(NA, nrow(mat_param))
    # mat_DM = array(NA, dim = list(length(modstep@coef), length(modstep@coef), nrow(mat_param)))
    # for(t_star in 1:nrow(mat_param)){
    #   print(paste0(t_star, "/", nrow(mat_param)))
    #   vec_param_t = mat_param[t_star,]
    #   vec_proba_t = mat_prob[t_star,]
    #   rownames(mat_DM) = colnames(mat_DM) = names(modstep@coef)
    #   for(param_i in names(modstep@coef)){
    #     vec_par_i_t = raw_params
    #     vec_par_i_t[param_i] = vec_param_t[param_i]
    #     vec_par_i_tplus1 = M_step(introd=vec_proba_t, modnum, prevfit=vec_par_i_t)[[1]]
    # 
    #     if(vec_param_t[param_i] - raw_params[param_i] != 0){
    #       for(param_j in names(modstep@coef)){
    #         mat_DM[param_i, param_j, t_star] = (vec_par_i_tplus1[param_j] - raw_params[param_j]) /(vec_param_t[param_i] - raw_params[param_i])
    #       }
    #     }else{
    #       print("Denominator = 0")
    #     }
    #   }
    #   print(mat_DM[,,t_star])
    #   penal_term[t_star] = tryCatch({sum(diag(mat_DM[,,t_star] - solve(diag(length(modstep@coef)) - mat_DM[,,t_star])))}, error=function(e){NA})
    # }
...
We create fake resistance data such that there is no relationship with recent AMU and no introduction: the resistance just follows a normal distribution of mean 0.4. This test has an interest for non bimodal data.
...
if(type_mod == "linear"){
  if(test_method_comp){
    ...
    res_data = as.data.frame(cbind(res_data_saved$S, rnorm(n_cyc, 0.4, 0.05), rnorm(n_cyc, 0.4, 0.05)))
  }
  
  hist(as.numeric(as.matrix(res_data)), nclass = 15, col = "royalblue4", main = "Histogram of all resistance data", xlab = "Resistance values")
  resamu(nwkseff = 8, quantuse=F, lag_amu_init = 1, expo="col")
}
...
if(type_mod == "linear"){
  ...
  if(test_method_comp){
    ...
    fit_model_1 = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "lambda2", "alpha", "beta", "teta", "delta", "eta", "Sd"), startvar = c("mu"))
  
    fit_model_7 = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c( "lambda1",  "lambda2",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "alpha",  "beta"))
...
if(type_mod == "linear"){
  ...
  bbmle::AIC(fit_model_1)
  
  print("AIC of model 7:")
  bbmle::AIC(fit_model_7)
  
  print("AIC of model 13:")
  bbmle::AIC(results_mod13[[1]]) #+ 2*results_mod13[["number_obs"]]
  
  print("AIC of model 19:")
  bbmle::AIC(results_mod19[[1]]) #+ 2*results_mod19[["number_obs"]]
  
  print("Anova with models 1 and 7 (for instance):")
  bbmle::anova(fit_model_1, fit_model_7)
}
...
We create fake resistance data such that there is no relationship with recent AMU but there are introductions: the resistance follows a normal distribution of mean 0.2 (no introduction) or 0.7 (an introduction occurred).
...
if(test_method_comp){
  ...
  res_data = as.data.frame(cbind(res_data_saved$S, rnorm(n_cyc, 0, 0.05) + base::sample(x = c(0.2,0.7), size = n_cyc, replace = T, prob = c(0.5,0.5)), rnorm(n_cyc, 0, 0.05) + base::sample(x = c(0.2,0.7), size = n_cyc, replace = T, prob = c(0.5,0.5))))
}
...
print("IC(H,Q) of model 13:")
vec_p = results_mod13[[3]]
vec_p[vec_p == 0] = 1e-7
vec_p[vec_p == 1] = 1-(1e-7)
bbmle::AIC(results_mod13[[1]]) + 2*sum(vec_p * log(vec_p) + (1-vec_p) * log(1-vec_p))

print("IC(H,Q) of model 19:")
vec_p = results_mod19[[3]]
vec_p[vec_p == 0] = 1e-7
vec_p[vec_p == 1] = 1-(1e-7)
bbmle::AIC(results_mod19[[1]]) + 2*sum(vec_p * log(vec_p) + (1-vec_p) * log(1-vec_p))

print("Anovas:")
bbmle::anova(fit_model_7, results_mod19[[1]])
bbmle::anova(results_mod13[[1]], results_mod19[[1]])

```

#### Scenario 3

We create fake resistance data such that there is a relationship with recent AMU, but not many "Low use, high resistance" data points that correspond to introductions.
...
if(test_method_comp){
  ...
  if(type_mod == "linear"){
    ...
    fake_res = run2(aim="pred", intr=0, mu=mu_fake, lambda1=0, lambda2=0, alpha=alpha_fake, beta=logit((beta_fake-1)/10), teta=0, delta=1, eta=0, Sd=0.1)[["obsdat"]]$predmod
    rm(res_data)
    ...
    res_data = res_data_saved
    fake_obsdat = run2(aim="pred", intr=0, mu=mu_fake, lambda1=0, lambda2=0, alpha=alpha_fake, beta=logit((beta_fake-1)/10), teta=0, delta=1, eta=0, Sd=0.1)[["obsdat"]]
    rm(res_data)
    fake_res = fake_obsdat$predmod
    fake_amu = fake_obsdat$amu_t
    
    low_use_high_res = NULL #sample(x = which(fake_amu < 0.5), size = 1)
    fake_res[low_use_high_res] = rnorm(length(low_use_high_res), 0.8, 0.05)

    low_use_low_res = setdiff(which(fake_amu < 0.5), low_use_high_res)
    fake_res[low_use_low_res] = rnorm(length(low_use_low_res), 0.2, 0.05)
    
    high_use_low_res = 35 #NULL #sample(x = which(fake_amu >= 0.5), size = 1)#42
    fake_res[high_use_low_res] = rnorm(length(high_use_low_res), 0.2, 0.05)
    
    high_use_high_res = setdiff(which(fake_amu >= 0.5), high_use_low_res)
    fake_res[high_use_high_res] = rnorm(length(high_use_high_res), 0.8, 0.05)
    ...
    res_data = as.data.frame(cbind(res_data_saved$S, matrix(fake_res, nrow=n_cyc, ncol=2, byrow=T)))
    rm(beta_fake, alpha_fake, fake_res, na_res_val, fake_obsdat, low_use_high_res, low_use_low_res, high_use_low_res, high_use_high_res)
  }
}
...
We create fake resistance data such that there is a relationship with recent AMU. We also add "Low use, high resistance" data points that correspond to introductions.
...
if(test_method_comp){
  ...
  if(type_mod == "linear"){
    ...
    introd_fake = sample(x = which(fake_res < 0.25), size = round(0.3*sum(fake_res < 0.25)))
    fake_res[introd_fake] = rnorm(length(introd_fake), 0.6, 0.05)
    
    fake_res[42] = rnorm(1, 0.1, 0.05)
    ...
    res_data = as.data.frame(cbind(res_data_saved$S, matrix(fake_res, nrow=n_cyc, ncol=2, byrow=T)))
    rm(beta_fake, alpha_fake, introd_fake, fake_res, na_res_val, fake_obsdat)
    ...
    low_use_high_res = sample(x = which(fake_amu < 0.5), size = round(0.3*sum(fake_res < 0.5)))
    fake_res[low_use_high_res] = rnorm(length(low_use_high_res), 0.8, 0.05)

    low_use_low_res = setdiff(which(fake_amu < 0.5), low_use_high_res)
    fake_res[low_use_low_res] = rnorm(length(low_use_low_res), 0.2, 0.05)
    
    high_use_low_res = 35 #sample(x = which(fake_amu >= 0.5), size = 1) #42
    fake_res[high_use_low_res] = rnorm(length(high_use_low_res), 0.2, 0.05)
...
We create fake resistance data such that there is a relationship with initial AMU, but not many "Low use, high resistance" data points that correspond to introductions.

```{r, fig.width=7, fig.height=5}
if(test_method_comp){
  load("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/Method validation/test_scenario_5.rdata")
  res_data = test_scenario_5[[1]]
}else{
  if(type_mod == "linear"){
    
    print("Scenario only used with logistic model.")
    ...
    res_data = res_data_saved
    fake_obsdat = run2(aim="pred", intr=0, mu=mu_fake, lambda1=0, lambda2=0, alpha=0, beta=1, teta=teta_fake, delta=logit((delta_fake-1)/4), eta=0, Sd=0.1)[["obsdat"]]
    rm(res_data)
    fake_res = fake_obsdat$predmod
    fake_amu = fake_obsdat$amu_init
    
    low_use_high_res = NULL #sample(x = which(fake_amu < 2), size = 1)
    fake_res[low_use_high_res] = rnorm(length(low_use_high_res), 0.8, 0.05)

    low_use_low_res = setdiff(which(fake_amu < 2), low_use_high_res)
    fake_res[low_use_low_res] = rnorm(length(low_use_low_res), 0.2, 0.05)
    
    high_use_low_res = NULL #sample(x = which(fake_amu >= 2), size = 1)#35
    fake_res[high_use_low_res] = rnorm(length(high_use_low_res), 0.2, 0.05)
    
    high_use_high_res = setdiff(which(fake_amu >= 2), high_use_low_res)
    fake_res[high_use_high_res] = rnorm(length(high_use_high_res), 0.8, 0.05)
...
hist(as.numeric(as.matrix(res_data)), nclass = 15, col = "royalblue4", main = "Histogram of all resistance data", xlab = "Resistance values")
resamu(nwkseff = 8, quantuse=F, lag_amu_init = 4, expo="col", var_to_plot = "amu_init")

```

We fit models 1, 4, 13, 16:

```{r, fig.width=6, fig.height=4}

if(test_method_comp){
  fit_model_1 = test_scenario_5[[2]]
  fit_model_4 = test_scenario_5[[3]]
  results_mod13 = test_scenario_5[[4]]
  results_mod16 = test_scenario_5[[5]]
}else{
  fit_model_1 = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "lambda2", "alpha", "beta", "teta", "delta", "eta", "Sd"), startvar = c("mu"))

  fit_model_4 = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "lambda2", "alpha", "beta", "eta", "Sd"), startvar = c("mu", "teta", "delta"))

  results_mod13 = EM_algo(modnum=13, numb_rep_algo=1)

  results_mod16 = EM_algo(modnum=16, numb_rep_algo=1)
  
  test_scenario_5 = list(res_data, fit_model_1, fit_model_4, results_mod13, results_mod16)
  save(test_scenario_5, file = "C:/Users/Jonathan/Desktop/test_scenario_5.rdata")
}

fitplot(fitted_model = fit_model_1, intro = 0)
fitplot(fitted_model = fit_model_4, intro = 0)
fitplot(fitted_model = results_mod13[[1]], intro = results_mod13[[3]])
fitplot(fitted_model = results_mod16[[1]], intro = results_mod16[[3]])

# rm(res_data)

```

Let us compare models' AIC. Minimum AIC should be model 4.

```{r}
print("AIC of model 1:")
bbmle::AIC(fit_model_1)

print("AIC of model 4:")
bbmle::AIC(fit_model_4)
...
print("IC(H,Q) of model 16:")
vec_p = results_mod16[[3]]
vec_p[vec_p == 0] = 1e-7
vec_p[vec_p == 1] = 1-(1e-7)
bbmle::AIC(results_mod16[[1]]) + 2*sum(vec_p * log(vec_p) + (1-vec_p) * log(1-vec_p))

print("Anovas:")
bbmle::anova(fit_model_4, results_mod16[[1]])
bbmle::anova(results_mod13[[1]], results_mod16[[1]])

```

#### Scenario 6

We create fake resistance data such that there is a relationship with initial AMU. We also add "Low use, high resistance" data points that correspond to introductions.

```{r, fig.width=7, fig.height=5}
if(test_method_comp){
  load("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/Method validation/test_scenario_6.rdata")
  res_data = test_scenario_4[[1]]
}else{
  if(type_mod == "linear"){
    ...
    low_use_high_res = sample(x = which(fake_amu < 2), size = round(0.3*sum(fake_res < 2)))
    fake_res[low_use_high_res] = rnorm(length(low_use_high_res), 0.8, 0.05)

    low_use_low_res = setdiff(which(fake_amu < 2), low_use_high_res)
    fake_res[low_use_low_res] = rnorm(length(low_use_low_res), 0.2, 0.05)
    
    high_use_low_res = NULL #42 #sample(x = which(fake_amu >= 2), size = 1) #35
    fake_res[high_use_low_res] = rnorm(length(high_use_low_res), 0.2, 0.05)
...
if(test_method_comp){
  fit_model_1 = test_scenario_6[[2]]
  fit_model_4 = test_scenario_6[[3]]
  results_mod13 = test_scenario_6[[4]]
  results_mod16 = test_scenario_6[[5]]
}else{
  fit_model_1 = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "lambda2", "alpha", "beta", "teta", "delta", "eta", "Sd"), startvar = c("mu"))
  ...
  test_scenario_6 = list(res_data, fit_model_1, fit_model_4, results_mod13, results_mod16)
  save(test_scenario_6, file = "C:/Users/Jonathan/Desktop/test_scenario_6.rdata")
}
...
Let us compare models' AIC. Minimum AIC should be model 16.
...
```{r}
res_data = res_data_saved
rm(res_data_saved, test_scenario_1, test_scenario_2, test_scenario_3, test_scenario_4, test_scenario_5, test_scenario_6, results_mod13, results_mod19, fit_model_1, fit_model_7)
```
...
if(alread_comp){
  if(ratio_auc){
    ...
    load(paste0("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/", definition_amu, " - ", ifelse(amu_model_is_quanti, "Quantitative", "Qualitative"), " - Exp - Diff/lfit_", definition_amu, "_isquanti", amu_model_is_quanti, ".rdata"))
  }
  
}else{
  lfit = list()
  
  lfit[[1]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "lambda2", "alpha", "beta", "teta", "delta", "eta", "Sd"), startvar = c("mu"))
  
  lfit[[2]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda2",  "alpha",  "beta",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "lambda1"))
  
  lfit[[3]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("alpha",  "beta",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "lambda1",  "lambda2"))
  
  lfit[[4]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1", "lambda2", "alpha", "beta", "eta", "Sd"), startvar = c("mu", "teta", "delta"))
  
  lfit[[5]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda2",  "alpha",  "beta", "eta", "Sd"), startvar = c("mu",  "lambda1",  "teta",  "delta"))
  
  lfit[[6]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("alpha",  "beta", "eta", "Sd"), startvar = c("mu",  "lambda1",  "lambda2",  "teta",  "delta"))
  
  lfit[[7]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1",  "lambda2",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "alpha",  "beta"))
  
  lfit[[8]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda2",  "teta",  "delta", "eta", "Sd"), startvar = c("mu",  "lambda1",  "alpha",  "beta"))
  
  lfit[[9]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("teta",  "delta", "eta", "Sd"), startvar = c("mu",  "lambda1",  "lambda2",  "alpha",  "beta"))
  
  lfit[[10]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda1",  "lambda2", "eta", "Sd"), startvar = c("mu",  "alpha",  "beta",  "teta",  "delta"))
  
  lfit[[11]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("lambda2", "eta", "Sd"), startvar = c("mu",  "lambda1",  "alpha",  "beta",  "teta",  "delta"))
  
  lfit[[12]] = rep_est(nrep=20, minusloglrep = run2_intr(0), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = "none", fixedvar = c("eta", "Sd"), startvar = c("mu", "lambda1", "lambda2",  "alpha",  "beta",  "teta",  "delta"))
...
if(alread_comp){
  if(ratio_auc){
    ...
    load(paste0("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/", definition_amu, " - ", ifelse(amu_model_is_quanti, "Quantitative", "Qualitative"), " - Exp - Diff/resulpar_", definition_amu, "_isquanti", amu_model_is_quanti, "_all.rdata"))
  }
  ...
  clusterExport(cl, c("thresh_res","type_mod","definition_amu","amu_model_is_quanti","E_step","M_step","rep_est","run2","run2_intr","resamu","n_cyc","res_data","weeks_samp","col_expo","col_expo_quanti","allab_expo","allab_expo_quanti"))
  resulpar_13to21 = parLapply(cl, X=13:21, fun=EM_algo, numb_rep_algo=1)
  stopCluster(cl)
  ...
  clusterEvalQ(cl, list(library(pracma), library(bbmle)))
  clusterExport(cl, c("thresh_res","type_mod","definition_amu","amu_model_is_quanti","E_step","M_step","rep_est","run2","run2_intr","resamu","n_cyc","res_data","weeks_samp","col_expo","col_expo_quanti","allab_expo","allab_expo_quanti"))
  resulpar_22to24 = parLapply(cl, X=22:24, fun=EM_algo, numb_rep_algo=1)
  stopCluster(cl)
...
for(m in 13:24){
  vec_p = resulpar[[m-12]][[3]]
  vec_p[vec_p == 0] = 1e-7
  vec_p[vec_p == 1] = 1-(1e-7)
  l_aic[m] = l_aic[m] + 2*sum(vec_p * log(vec_p) + (1-vec_p) * log(1-vec_p))
}
...
```{r}
max_AICweight_model = which.max(tab_aic_ll$AIC_weights)
print(paste0("Model ", max_AICweight_model))
...
```{r, fig.width=6, fig.height=4}
if(max_AICweight_model %in% (1:12)){
  
  fitplot(fitted_model = lfit[[max_AICweight_model]], intro = 0)
  
}else if(max_AICweight_model %in% (13:24)){
  
  fitplot(fitted_model = lfit[[max_AICweight_model]], intro = resulpar[[max_AICweight_model-12]][[3]])
}
...
```{r}
print(tab_aic_ll$AIC_weights[max_AICweight_model] /tab_aic_ll$AIC_weights[13])
bbmle::anova(lfit[[max_AICweight_model]], lfit[[13]])
...
```{r}
cum_aic_wght = data.frame(mod = 1:24, AICw = l_aic_weights, intr_poss = NA, rec_amu = NA, init_amu = NA, first_samp = NA, prev_samp = NA)

cum_aic_wght[13:24, "intr_poss"] = 0.25
cum_aic_wght[c(7:12, 19:24), "rec_amu"] = 0.2
cum_aic_wght[c(4:6, 10:12, 16:18, 22:24), "init_amu"] = 0.15
cum_aic_wght[seq(2,23,3), "first_samp"] = 0.1
cum_aic_wght[seq(3,24,3), "prev_samp"] = 0.05

cum_aic_wght = cum_aic_wght[order(cum_aic_wght$AICw, decreasing = T),]
cum_aic_wght$cum_AICw = cumsum(cum_aic_wght$AICw)
cum_aic_wght$mod = factor(cum_aic_wght$mod, levels = cum_aic_wght$mod)

cum_aic_wght = melt(cum_aic_wght, id.vars=c("mod", "AICw", "cum_AICw"), variable.name="var", value.name="position")

val_line = 0.95

p = ggplot(data = cum_aic_wght, aes(x = mod, y = cum_AICw, group = 1, na.rm = TRUE))
p = p + ylim(0,1) + xlab("Models") + ylab("Cumulative AIC weights")
p = p + theme_bw()
p = p + geom_line() + geom_point(size = 2)
p = p + geom_hline(yintercept = val_line, col = "brown3", size = 1, linetype = "dashed")
p = p + geom_label(x = 12, y = val_line, label = val_line, col = "brown3", size = 3.5)
...
p = p + scale_colour_brewer(type="qual", palette=1,
                           ...
                           labels = c("Introductions", "Recent AMU", "Initial AMU", "First sample", "Previous sample"))
p
...
```{r}
max_AICweight_intro_model = max_AICweight_model -12
...
coef_moran = function(k, decay_fun){
  ...
  res = Moran.I(x = prob_intro_farms$Prob, weight = weight_matrix, alternative = "greater")
...
dist_geo = spDists(x = as.matrix(x = prob_intro_farms[,c("Long", "Lat")]), longlat = T) # Must stay outside the function "local_moran"

local_moran = function(decay_param, decay_fun, p_val, style_weight){
  
  set.ZeroPolicyOption(TRUE)
  prob_intro_farms$Lat = prob_intro_farms$Lat + runif(length(prob_intro_farms$Lat), 0, 0.00001)
  prob_intro_farms$scaled_prob = as.numeric(scale(prob_intro_farms$Prob))
  
  if(decay_fun == "exp"){
    
    neighbors = dnearneigh(x = as.matrix(prob_intro_farms[,c("Long","Lat")]), d1 = 0, d2 = 1000, longlat = T)
    ...
    neighbors = knn2nb(knearneigh(x = as.matrix(prob_intro_farms[,c("Long","Lat")]), k = decay_param, longlat = T))
    listw_for_test = nb2listw(neighbors, style = style_weight)
    
  }else if(decay_fun == "thresh"){
    
    neighbors = dnearneigh(x = as.matrix(prob_intro_farms[,c("Long","Lat")]), d1 = 0, d2 = decay_param, longlat = T)
    listw_for_test = nb2listw(neighbors, style = style_weight)
  }

  p.val.moran = moran.test(x = prob_intro_farms$scaled_prob, listw = listw_for_test)$p.value
  res.local.moran = localmoran(x = prob_intro_farms$scaled_prob, listw = listw_for_test)
  prob_intro_farms$lag = lag.listw(listw_for_test, prob_intro_farms$scaled_prob)
  prob_intro_farms$pval = res.local.moran[,5]
  ...
  p1 = p1 + geom_jitter(data = prob_intro_farms, aes(x = Long, y = Lat, fill = cluster), col = "black", shape = 21, size = 3, width = 0.006, height = 0.006)
  ...
  # p1 = p1 + scale_fill_manual("Cluster:", values = c("High-high (p<0.05)"="red", "Low-low (p<0.05)"="blue", "High-high (p<0.1)"="lightpink", "Low-low (p<0.1)"="skyblue2", "High-low"="forestgreen", "Low-high"="forestgreen", "p>0.1"="white"))
  p1 = p1 + scale_fill_manual("Cluster:", values = c("High-high (p<0.05)"="red", "Low-low (p<0.05)"="blue", "High-high (p<0.1)"="lightpink", "Low-low (p<0.1)"="skyblue2", "High-low"="forestgreen", "Low-high"="forestgreen", "Not significant"="white"))
...
pB = local_moran(decay_param = 1, decay_fun = "exp", p_val = 0.05, style_weight = "W")[[2]]

com_plot = ggarrange(pA, pB, labels = c("A", "B"))
plot(com_plot)
