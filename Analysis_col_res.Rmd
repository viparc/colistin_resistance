...
viparc_data_quali = read.csv("C:/Users/Jonathan/Desktop/Programmes/colistin_resistance/viparc_qualitative.csv") # read.csv("https://raw.githubusercontent.com/viparc/colistin_resistance/master/data/viparc_qualitative.csv")
viparc_data_quanti = read.csv("C:/Users/Jonathan/Desktop/Programmes/colistin_resistance/viparc_quantitative.csv") # read.csv("https://raw.githubusercontent.com/viparc/colistin_resistance/master/data/viparc_quantitative.csv")
...
if(rownames(allab_expo)!=rownames(res_data) | rownames(col_expo)!=rownames(res_data)){print("Row names of AMU data and resistance data are not consistent.")}
...
```{r, fig.width=8, fig.height=7, fig.retina=2}
...
for (i in 1:n_cyc){
  for (j in 1:max(which(! is.na(col_expo[i,])))){
    ...
    if ((!(is.na(col_expo[i,j]))) & (allab_expo[i,j] == 1)){
    ...
    if (any(weeks_samp[i,] == j, na.rm=T)){
      if (is.finite(res_data[i, which(weeks_samp[i,] == j)])){
        lines(x = c(j+0.5, j-0.5), y = c(i-0.5, i+0.5), col = "black", lwd = 2)
        lines(x = c(j-0.5, j+0.5), y = c(i-0.5, i+0.5), col = "black", lwd = 2)
      }
    }
  }
...
legend(x=0, y=n_cyc+4, inset=0.5, legend=c("Antimicrobials used", "No antimicrobials used"), fill=c("coral3", "bisque"), cex = 0.8, bty = "n")
...
p2 = p2 + geom_label_repel(data = for_fig_panel[for_fig_panel$Time == 70,], xlim = c(3, 4), ylim = c(0.6, 1.4), direction = "y", hjust = 0)
p2 = p2 + theme_classic() + theme(legend.position="none",
                             plot.background=element_rect(colour ="black"),
                             plot.margin = unit(c(0.2, 0.2, 0.1, 0.1), "cm"))

print(p2, vp=viewport(width = 0.26, height = 0.26, x = 0.99, y = 0.35, just = c("right", "bottom")))

lines(x=c(weeks_samp["0625",3], 22.85), y=c(13, 13), lwd=1)
...
resamu = function(nwkseff, quantuse, tempo_amu, ignore.nul=F, lag_amu_init, obj="plot", expo, var_to_plot = "amu_t"){
  ...
  for(cyc_i in 1:n_cyc){
    for(time_i in 2:3){
      ...
      first_amu_week_consid = as.numeric(weeks_samp[cyc_i, time_i] - nwkseff - 1) + (1-almost_one)
  ...
  obtab$resquali = as.numeric(obtab$resquanti >= 0.5)
...
```{r}
rep_est = function(nrep=3, minusloglrep, methodrep, controlrep, prev_est, fixedvar, startvar){
  modrep = list()
...
M_step <- function(introd, modnum, prevfit) {
  ...
  #####
  if(modnum == 1){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2",  "alpha",  "beta",  "teta",  "delta", "eta"), startvar = c("mu"))
  }else if(modnum == 2){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2",  "alpha",  "beta",  "teta",  "delta", "eta"), startvar = c("mu",  "lambda1"))
  }else if(modnum == 3){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("alpha",  "beta",  "teta",  "delta", "eta"), startvar = c("mu", "lambda1",  "lambda2"))
  }else if(modnum == 4){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2",  "alpha",  "beta", "eta"), startvar = c("mu",  "teta",  "delta"))
  }else if(modnum == 5){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2",  "alpha",  "beta", "eta"), startvar = c("mu",  "lambda1",  "teta",  "delta"))
  }else if(modnum == 6){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("alpha",  "beta", "eta"), startvar = c("mu",  "lambda1",  "lambda2",  "teta",  "delta"))
  }else if(modnum == 7){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2",  "teta",  "delta", "eta"), startvar = c("mu",  "alpha",  "beta"))
  }else if(modnum == 8){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2",  "teta",  "delta", "eta"), startvar = c("mu",  "lambda1",  "alpha",  "beta"))
  }else if(modnum == 9){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("teta",  "delta", "eta"), startvar = c("mu",  "lambda1",  "lambda2",  "alpha",  "beta"))
  }else if(modnum == 10){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda1",  "lambda2", "eta"), startvar = c("mu",  "alpha",  "beta",  "teta",  "delta"))
  }else if(modnum == 11){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("lambda2", "eta"), startvar = c("mu",  "lambda1",  "alpha",  "beta",  "teta",  "delta"))
  }else if(modnum == 12){
    modstep = rep_est(minusloglrep = run2_intr(introd), methodrep = "Nelder-Mead", controlrep = list(maxit = 5000), prev_est = prevfit, fixedvar = c("eta"), startvar = c("mu",  "lambda1",  "lambda2",  "alpha",  "beta",  "teta",  "delta"))
  }
  #####
...
```{r}
EM_algo <- function(modnum, numb_rep_algo=5, tol=0.01, max.step=10000, plot_step=T, init_p = 0.01) {

  repetalgo = list()
  repetalgo_mincomploglik = c()
  ...
  for(algorep_ite in 1:numb_rep_algo){
    ...
    while((step < 4) | ((counting_for_convergence <5) & (step <= max.step+1))){
      ...
      #   raw_params["eta"] = 5
      # }
      old.mincomploglik = mincomploglik
      mincomploglik = output_mstep[[2]]
      ...
      if (abs(mincomploglik - old.mincomploglik) < tol){
    ...
    repetalgo[[algorep_ite]] = list(modstep, raw_params, introductions, mincomploglik, param_true_format, number_obs=length(obs_res), cycle_id, obs_ll)
    repetalgo_mincomploglik[algorep_ite] = mincomploglik
  }
  ...
  repetalgo[[which.min(repetalgo_mincomploglik)]]
}
```
...
if(alread_comp){
  ...
  load(paste0("C:/Users/Jonathan/Desktop/Resultats simulations/colistin_resistance/", definition_amu, " - ", ifelse(amu_model_is_quanti, "Quantitative", "Qualitative"), " - ", effect_recent_amu, "/c=", conc_used, "/resulpar_all_", definition_amu, "_isquanti", amu_model_is_quanti, "_", effect_recent_amu, "_c", conc_used, ".rdata"))
  ...
  clusterEvalQ(cl, list(library(pracma), library(bbmle)))
  clusterExport(cl, c("conc_used","effect_recent_amu","almost_one","type_mod","definition_amu","amu_model_is_quanti","E_step","M_step","rep_est","run2","run2_intr","resamu","n_cyc","res_data","weeks_samp","col_expo","col_expo_quanti","allab_expo","allab_expo_quanti"))
  resulpar = parLapply(cl, X=1:12, fun=EM_algo, numb_rep_algo=10, plot_step=F)
  stopCluster(cl)
...
```
We load sampling dates data:

```{r}
# datsamp = data.frame(cyc = c("0681", "0721", "0211", "0271", "0901", "0931", "0961", "0082", "0062", "1031", "0491", "0501", "0511", "0023", "0571", "0581", "0463", "0523", "0625"),
#                      date_first = as.Date(c("08/11/2016", "23/11/2016", "04/12/2016", "25/12/2016", "06/01/2017", "13/01/2017", "03/02/2017", "22/06/2017", "03/05/2017", "09/05/2017", "12/05/2017", "20/05/2017", "02/06/2017", "26/09/2017", "22/10/2017", "17/10/2017", "19/12/2017", "25/05/2018", "05/07/2018"), format="%d/%m/%Y"))
# 
# datsamp$date_sec = NA
# datsamp$date_thir = NA
# 
# for(id_i in rownames(weeks_samp)){
#   datsamp$date_sec[datsamp$cyc == id_i] = datsamp$date_first[datsamp$cyc == id_i] + 7* (weeks_samp[id_i,2] - weeks_samp[id_i,1])
#   datsamp$date_thir[datsamp$cyc == id_i] = datsamp$date_first[datsamp$cyc == id_i] + 7* (weeks_samp[id_i,3] - weeks_samp[id_i,1])
# }
# 
# datsamp$cyc = substr(datsamp$cyc, 1, 3)
# 
# prob_intro_farms$date = NA
# for(cyc_i in datsamp$cyc){
#   prob_intro_farms$date[which(prob_intro_farms$Farm == cyc_i)[1]] = datsamp$date_sec[(datsamp$cyc == cyc_i)]
#   prob_intro_farms$date[which(prob_intro_farms$Farm == cyc_i)[2]] = datsamp$date_thir[(datsamp$cyc == cyc_i)]
# }
# prob_intro_farms$date = as.Date(prob_intro_farms$date, origin="1970-01-01")
...
sf_stud_reg = sf_file[sf_file$D_CODE %in% c(872, 873),]
sf_file_df = fortify(sf_stud_reg)

sf_stud_reg@data$id = names(table(sf_file_df$id))

sf_file_df <- base::merge(x=sf_file_df, y=sf_stud_reg@data[,c(1:9, which(colnames(sf_stud_reg@data) == "id"))], by="id", all.x=T)
sf_file_df$C_NAMEEN = as.character(sf_file_df$C_NAMEEN)
sf_file_df$C_NAMEEN[sf_file_df$C_CODE == 30037] = "My An Town"
sf_file_df$C_NAMEEN[sf_file_df$C_CODE == 30076] = "My Tho Town"

# popdat = readRDS("C:/Users/Jonathan/Desktop/Programmes/colistin_resistance/census2019.rds")
# popdat = aggregate(n ~ province + district + commune, data=popdat, FUN=sum)
# popdat = popdat[popdat$district %in% unique(popdat$district)[grep("ao Lã|p M", unique(popdat$district))],]
# for(i in 1:nrow(popdat)){print(popdat$commune[i]) ; print(popdat$n[i])}

datdens = as.data.frame(read_excel("C:/Users/Jonathan/Desktop/Programmes/colistin_resistance//Table_data_commune.xlsx"))

sf_file_df = merge(x=sf_file_df, y=datdens, by.x="C_NAMEEN", by.y="Commune")

# ggplot() + geom_polygon(data = VN, aes(x = long, y = lat, group = group), color = 'black', fill = 'lemonchiffon', size = .2) + geom_polygon(data = VN2, aes(x = long, y = lat, group = group), color = 'black', fill = NA, size = 1.2)
...
pA = pA + scale_fill_gradient2(name = "Probability of unexplained infection:", low = "dodgerblue", mid = "white", high = "red", midpoint = 0.5, limits = c(0, 1))
...
pA = pA  + guides(fill = guide_colourbar(title.position="top"))
pA = pA + blank()


pB = local_moran(decay_param = 2.3, decay_fun = "thresh", p_val = 0.05, style_weight = "W", vis_spat_weight = F)[[2]]
...
# com_plot = ggarrange(pC, pA, pB, pD, nrow = 2, ncol = 2, labels = c("A", "B", "C", "D))
com_plot = ggarrange(pC, pA, pB, nrow = 1, ncol = 3, labels = c("A", "B", "C"))
plot(com_plot)
