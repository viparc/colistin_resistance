...
run2 = function(aim="est", intr=introd, distrib_res="normal", mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd){
  ...
  if(aim == "est"){
    if(distrib_res == "normal"){
      lik_intro = dnorm(x=obsdat$auc, mean=obsdat$predmod_intro, sd=Sd)
      lik_nointro = dnorm(x=obsdat$auc, mean=obsdat$predmod_nointro, sd=Sd)
    ...
    return(list(obsdat = obsdat[,c("auc", "amu_init", "amu_t", "introduc", "predmod")], minll = -sum(dnorm(x=obsdat$auc, mean=obsdat$predmod, sd=Sd, log=T)), true_param = c(mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd)))
  }
}
...
fitplot = function(fitted_model, intro){
  ...
  p = ggplot(obs_vs_pred[[1]], aes(x = auc, y = predmod, fill = true_fit_coef["alpha"]*amu_t + true_fit_coef["teta"]*amu_init, col = introduc))
...
results = EM_algo(modnum=19, numb_rep_algo=1)
...
l_aic[13:24] = l_aic[13:24] + 49 #resulpar[[1]][["number_obs"]]

# l_aic = exp(-(l_aic - min(l_aic))/2) / sum(exp(-(l_aic - min(l_aic))/2))

plot_aic_ll = data.frame(mod = as.factor(1:24), AIC = l_aic, MinusLogLikelihood = -l_loglik)
...
# with - without introductions:
mean(l_aic[c(13:24)]) - mean(l_aic[-c(13:24)])

# with - without initial AMU:
mean(l_aic[c(4:6, 10:12, 16:18, 22:24)]) - mean(l_aic[-c(4:6, 10:12, 16:18, 22:24)])

# with - without recent AMU:
mean(l_aic[c(7:12, 19:24)]) - mean(l_aic[-c(7:12, 19:24)])

# with - without initial resistance:
mean(l_aic[c(2,3,5,6,8,9,11,12,14,15,17,18,20,21,23,24)]) - mean(l_aic[-c(2,3,5,6,8,9,11,12,14,15,17,18,20,21,23,24)])

# with - without previous resistance

# Step 2 with intro:
l_aic_withintro = l_aic[13:24]

# with intro, with - without initial AMU:
mean(l_aic_withintro[c(4:6, 10:12)]) - mean(l_aic_withintro[-c(4:6, 10:12)])

# with intro, with - without recent AMU:
mean(l_aic_withintro[c(7:12)]) - mean(l_aic_withintro[-c(7:12)])

# with intro, with - without initial resistance:
mean(l_aic_withintro[c(2,3,5,6,8,9,11,12)]) - mean(l_aic_withintro[-c(2,3,5,6,8,9,11,12)])

# Step 3 with intro and recent AMU:

# with intro and recent AMU, with - without initial AMU:
mean(l_aic[c(22:24)]) - mean(l_aic[c(19:21)])

# with intro and recent AMU, with - without initial resistance:
mean(l_aic[c(20,21,23,24)]) - mean(l_aic[c(19,22)])
