...
if(type_res_data == "growth_curves"){
  ...
  distrib_auc = data.frame(conc = numeric(0), val = numeric(0), thresh = numeric(0))
  for(c in c(1, 2, 4)){
    ...
    vect_val = distrib_auc$val[distrib_auc$conc == c]
    set.seed(78)
    gmm = normalmixEM(vect_val, k = 2)
    dif_distrib = function(x){
      gmm$lambda[1] * dnorm(x, gmm$mu[1], gmm$sigma[1]) - gmm$lambda[2] * dnorm(x, gmm$mu[2], gmm$sigma[2])
    }
    thresh_res_this_conc = uniroot(f = dif_distrib, lower=-50, upper=150)$root
    
    distrib_auc = rbind(distrib_auc, data.frame(conc = c, val = VdE_A[[2]], thresh = thresh_res_this_conc))
  }
  ...
  p = p + geom_vline(aes(xintercept = thresh), size = 1, col = "red", linetype = "dashed")
  ...
  thresh_res = distrib_auc$thresh[distrib_auc$conc == conc_used]
  
  rm(c, VdE_A_score, VdE_A, p)
}
```
...
# 
# set.seed(1)
# vect_val <- distrib_auc$val[distrib_auc$conc == 1]
# mixmdl <- normalmixEM(vect_val, k = 2)
# 
# 
# ggplot(data.frame(x = vect_val)) +
...
# 
# dif_distrib = function(x){
#   mixmdl$lambda[1] * dnorm(x, mixmdl$mu[1], mixmdl$sigma[1]) - mixmdl$lambda[2] * dnorm(x, mixmdl$mu[2], mixmdl$sigma[2])
# }
# 
# thresh_res = uniroot(f = dif_distrib, lower=-50, upper=150)$root

```

We select the concentration for which the standard deviation is the highest, or for which the "van der Eijk's A" statistic is closest to 0, therefore concentration 1 in this case. We average the 2 repetitions of a same sample, and reshape the data to obtain `auc_ob`.

We standardize the AUC values to range between 0 (lowest value) and 1 (highest value).
...
if(type_res_data == "growth_curves"){
  ...
  thresh_res = (thresh_res - min(auc_ob, na.rm=T))/(max(auc_ob, na.rm=T) - min(auc_ob, na.rm=T))
  auc_ob = (auc_ob - min(auc_ob, na.rm=T))/(max(auc_ob, na.rm=T) - min(auc_ob, na.rm=T))
  
  auc_ob = auc_ob[which(! rownames(auc_ob) %in% c("0351", "0381", "0391", "0765", "0771")),]
  ...
  for (j in 1:max(which(! is.na(col_expo[i,])))){
    ...
    if (any(weeks_samp[i,] == j, na.rm=T)){
      if (is.finite(res_data[i, which(weeks_samp[i,] == j)])){
        # rect(xleft = j-0.07, xright = j+0.07, ybottom = i-0.5, ytop = i+0.5, col = "black", border="black")
        # lines(x = c(j-0.5, j+0.5), y = c(i-0.5, i+0.5), col = "black")
        # lines(x = c(j-0.5, j+0.5), y = c(i+0.5, i-0.5), col = "black")
...
p = p + geom_text(aes(x=6.1, y=0.12, label = as.character(expression(paste("t"["ij"]," -",~beta)))), parse = T, size=5, col="dodgerblue4")
...
run2 = function(aim="est", intr=introd, mu, lambda1, lambda2, alpha, beta, teta, delta, eta, Sd){
  ...
  obsdat = resamu(nwkseff=beta, quantuse=amu_model_is_quanti, tempo_amu="linear", ignore.nul=F, lag_amu_init=delta, obj="est", expo=definition_amu)
...
$$Q(\theta,\theta^t) = E[ln \, P(Y,Z|\theta) \quad |Y,\theta^t]$$

$$\begin{align*}
Q(\theta,\theta^t)  &= \sum_{m} E[ln \, P(Y_m,Z_m|\theta) \quad |Y_m,\theta^t] \\
                    &= \sum_{m} [P(Z_m=0|Y_m,\theta^t).ln \, P(Z_m=0,Y_m|\theta) \: + \: P(Z_m=1|Y_m,\theta^t).ln \, P(Z_m=1,Y_m|\theta)]
\end{align*}$$
...
E_step <- function(observed, params, prev_p_intro) {
  ...
  pred_intro = run2(aim="pred", intr=1, mu=params[["mu"]], lambda1=params[["lambda1"]], lambda2=params[["lambda2"]], alpha=params[["alpha"]], beta=params[["beta"]], teta=params[["teta"]], delta=params[["delta"]], eta=params[["eta"]], Sd=params[["Sd"]])[["obsdat"]]$predmod
  ...
  if(type_mod == "linear"){
    ...
    exp_intro = pmin(0.999, exp_intro)
    exp_intro = pmax(0.001, exp_intro)
    exp_nointro = pmin(0.999, exp_nointro)
    exp_nointro = pmax(0.001, exp_nointro)
    prob_intro = exp_intro / (exp_intro + exp_nointro)
  }else if(type_mod == "logis"){
    observed = as.numeric(observed >= thresh_res)
    pred_intro = pmax(pmin(pred_intro, 0.999), 0.001)
    pred_nointro = pmax(pmin(pred_nointro, 0.999), 0.001)
...
```{r}
EM_algo <- function(modnum, numb_rep_algo=5, tol=0.001, max.step=100000000000, plot_step=T, init_p = 0.01) {
...
if(alread_comp){
  ...
  resulpar_13to21 = parLapply(cl, X=13:21, fun=EM_algo, numb_rep_algo=1, plot_step=F)
  stopCluster(cl)
  ...
  clusterEvalQ(cl, list(library(pracma), library(bbmle)))
  clusterExport(cl, c("thresh_res","type_mod","definition_amu","amu_model_is_quanti","E_step","M_step","rep_est","run2","run2_intr","resamu","n_cyc","res_data","weeks_samp","col_expo","col_expo_quanti","allab_expo","allab_expo_quanti"))
  resulpar_22to24 = parLapply(cl, X=22:24, fun=EM_algo, numb_rep_algo=1, plot_step=F)
  stopCluster(cl)
...
for(m in 13:24){
  l_Q_fct[m-12] = l_Q_fct[m] = -as.numeric(resulpar[[m-12]][[4]])
  l_numb_par[m-12] = l_numb_par[m] = length(resulpar[[m-12]][[1]]@coef)
  
  vec_p = resulpar[[m-12]][[3]]
  vec_p[vec_p == 0] = 1e-7
  vec_p[vec_p == 1] = 1-(1e-7)
  
  l_H_fct[m-12] = l_H_fct[m] = sum(vec_p * log(vec_p) + (1-vec_p) * log(1-vec_p))
}

l_IC_HQ = -2*l_Q_fct + 2*l_H_fct + 2*l_numb_par

tab_aic_ll = data.frame(mod = as.factor(1:24), Q_fct = l_Q_fct, H_fct = l_H_fct, numb_par = l_numb_par, IC_HQ = l_IC_HQ)
...
rm(p, plot_aic_ll, l_Q_fct, l_H_fct, l_numb_par)
```
...
prob_intro_farms$Obs_res = as.character(fitplot(fitted_model = lfit[[max_AICweight_model]], intro = resulpar[[max_AICweight_model-12]][[3]]))
```
